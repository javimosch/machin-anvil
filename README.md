...
- **Hidden-state pooling** — `ANVIL_POOL=max|mean` + `ANVIL_POOL_LAYER` (`-2` default, `final` = post-norm) feeds heads a pooled feature over the last user turn instead of the final position — measured +15pp on long-context fleet labels vs last-token. `mhd2` heads declare their trained pooling in a flags word (`kind | (layer+1)<<4`); a config mismatch warns at boot. Costs a full per-token prefill (no batch/cache), so head calls on long prompts run slower — the accuracy is the point.
