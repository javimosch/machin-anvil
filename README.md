...
- **Auth + per-tenant heads** — `ANVIL_KEYS="k1,k2"` gates all `/v1/*` behind `Authorization: Bearer` (`/health` stays open; unset = open, backward compatible). `ANVIL_TENANTS="k1:a.head,k2:b.head"` maps a key to its own decision head, preloaded at startup and swapped per request under the model lock; unmapped keys get `ANVIL_HEAD`. Route customers to different heads without restarting or fine-tuning the trunk.
- **Mixture of experts** — `ANVIL_GATE=<gate.head>` + `ANVIL_EXPERTS="dom:a.head,dom2:b.head"` turns `/v1/route` into a two-stage dispatcher: the gate head picks the domain on the same prefill, the matching expert head picks the route. Unmapped domains fall back to `ANVIL_HEAD`; `ANVIL_GATE_MINCONF` (or per-request `min_gate_conf`) delegates with `low_gate_confidence` when the gate itself is unsure. Response gains `domain`, `gate_confidence`, `expert`. A request may also send `"expert":"<domain>"` to pin its lane — the gate is skipped (the caller already knows its domain; the gate exists to classify *mixed* front-door traffic), noul/score OOD vetoes are bypassed, and the expert's own `escalate` class is the abstention path. Unknown pins return 400.
- **Per-head pooled features** — `mhd2` heads self-describe the feature they were trained on in a flags word (`pool_kind | (layer+1)<<4`, 0 = last-token): each declared `(kind, layer)` tap gets its own accumulator over the last user turn, and `score_head` reads that head's feature. Mixed configs coexist on one instance — a last-token gate, a max-pooled fleet expert, and mean-pooled helpdesk/rpg heads run on a single prefill (verified: 3 distinct taps live on one server). `ANVIL_POOL=max|mean` + `ANVIL_POOL_LAYER` remains as a force-override onto every head (warns on flag mismatch). Sweep verdicts: **mean pooling wins on short synthetic rows, max on long prose, last-token holds on canonical short commands** — pick per head, not per server. Costs a full per-token prefill (no batch/cache) once any pooled head is loaded, so head calls run slower — the accuracy is the point.
- **Delegation** — `ANVIL_DELEGATE_URL` (plus `ANVIL_DELEGATE_KEY`/`MODEL`/`SYS`) points `/v1/route` at any OpenAI-compatible upstream: when the dispatcher decides to `delegate` and `execute:true`, the request is forwarded after releasing the model lock, and the upstream answer returns in `content`/`via`. `max_tokens` is always bounded (default 512).
