...
- **Mixture of experts** — `ANVIL_GATE=<gate.head>` + `ANVIL_EXPERTS="dom:a.head,dom2:b.head"` turns `/v1/route` into a two-stage dispatcher: the gate head picks the domain on the same prefill, the matching expert head picks the route. Unmapped domains fall back to `ANVIL_HEAD`; `ANVIL_GATE_MINCONF` (or per-request `min_gate_conf`) delegates with `low_gate_confidence` when the gate itself is unsure. Response gains `domain`, `gate_confidence`, `expert`.
